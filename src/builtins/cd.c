...
/*   Updated: 2025/02/21 15:56:50 by ggroff-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	update_env_var(t_shell *shell, const char *name, const char *value)
{
	...
	shell->env_copy = ft_realloc_array(shell->env_copy, i + 2, new_var, shell);
	if (!shell->env_copy)
		return (free(new_var));
	shell->env_copy[i] = new_var;
...
int	the_cd(char **args, t_shell *shell)
{
	...
	ret = change_directory(new_path_dir, shell, atl_dir);
	return (free(atl_dir), ret);
}
